...
public:
    ...
    inline Builder& action(Action::Func func) {
        ...
        // Apply decorators in reverse order (last added first)
    ...
    inline Builder& inverter() {
        return decorator(decorators::Inverter());
    }
    
    inline Builder& succeeder() {
        return decorator(decorators::Succeeder());
    }
    
    inline Builder& failer() {
        return decorator(decorators::Failer());
    }
    
    inline Builder& repeat(int maxTimes = -1) {
        return decorator(decorators::Repeat(maxTimes));
    }
    
    inline Builder& repeater(int maxTimes = -1) {
        return repeat(maxTimes);
    }
